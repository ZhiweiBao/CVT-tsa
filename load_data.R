...
u_A_train <- u_A[1:2000]
u_B_train <- u_B[1:2000]
u_C_train <- u_C[1:2000]
...
u_A_mul <- msts(u_A_train, seasonal.periods=c(16, 96))
model_A <- mstl(u_A_mul)
...
u_B_mul <- msts(u_B_train, seasonal.periods=c(16, 96))
model_B <- mstl(u_B_mul)
# model_B %>% autoplot()
...
u_C_mul <- msts(u_C_train, seasonal.periods=c(16, 96))
model_C <- mstl(u_C_mul)
# model_C %>% autoplot()
...
write.csv(ts_seasadj, './data/re_error_seasadj.csv',row.names = TRUE)
